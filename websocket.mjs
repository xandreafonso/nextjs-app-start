...
fastify.ready().then(() => {
    fastify.io.on("connection", async (socket) => {
        ...
        socket.on("disconnect", (reason) => {
            if (!socket.user) {
                fastify.log.warn(`Desconexão de socket ${socket.id} não autenticado. Motivo: ${reason}.`)
                return
            }
            
            fastify.log.info(`Usuário desconectado ${socket.user.email}. Motivo: ${reason}.`)
            ...
            if (socketUser) {
                ...
                if (socketUser.sockets.length === 0) {
                    ...
                    fastify.log.info(`Dispositivo do usuário ${socket.user.email} removido.`)
                }
            }
        })
    })
